...
import { LayoutProps } from "../../../app/(protected)/template";
...
const TopBar = ({ canGoBack }: LayoutProps) => {
  ...
  return (
    <div className="xl:hidden fixed bg-white w-full h-16 max-w-xl justify-center text-lg px-10 font-medium text-stone-800 border-b top-0 flex items-center z-10">
      {canGoBack ? (
        <button onClick={onClick} className="absolute left-4" data-testid="back-button">
          <svg
            className="w-6 h-6"
            fill="none"
            stroke="currentColor"
      ...
      <button onClick={() => router.push("/account")} className="absolute right-4 hover:animate-spin">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
