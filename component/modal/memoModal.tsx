...
const MemoModal = ({
  ...
  return (
    <>
      {type === "read" && !isEdit ? (
        <div className={cls("w-full h-52 bg-neutral-100 p-4 rounded-xl overflow-y-auto outline-none")}>{savedMemo}</div>
      ) : (
        <textarea
          ref={memoRef}
          name="memo"
      ...
      {isEdit ? (
        <div className="flex justify-end space-x-3">
          <Button text="완료" className="mt-3 !w-1/6" type="button" onClick={handleClick} />
        </div>
      ) : (
        <div className="flex justify-end space-x-3">
          <Button text="삭제" className="mt-3 red_btn !w-1/6 py-3" type="button" onClick={handleClickDelete} />
          <Button
