...
const Pay = React.memo(({ member, paymentType }: { member: IMemberWithSchedules; paymentType: PaymentType }) => {
  ...
  return (
    ...
    >
      ...
      <td>{formatCurrency((member.Schedule && member.Schedule[0]?.lessonFee) || "") || "-"}</td>

      {paymentType === PaymentType.DIFFERENT ? <td>{member.payDay} 일</td> : null}
