...
interface WorkerListProps {
  ...
  setInitValue?: React.Dispatch<React.SetStateAction<{ id: string; name: string }>>;
  className?: string;
  ...
  useEffect(() => {
    const fetchWorkerList = async () => {
      ...
      const filterWorkerList = workerList.filter((worker) => {
        if (selectedDay) {
          return selectedDay.some((day) => worker?.dayOfWeek?.includes(day.toString()));
        } else {
      ...
      setInitValue && setInitValue({ id: firstWorkerId, name: firstWorkerName });
      ...
      {!loading ? (
        isOnly && onChange ? (
          ...
          <Input
            ...
            name={"worker"}
            className={cls("h-16 xl:text-lg border-b-0 xl:border-b", className || "")}
