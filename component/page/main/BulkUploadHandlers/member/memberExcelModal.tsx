...
import { cls, generateDateOptions, scheduleValid } from "../../../../../libs/client/utils";
import {
  BIRTH_REGEX,
  COMMISSION_REGEX,
  DATE_REGEX,
    ...
    try {
      const errorIndexes = listData.reduce((acc: number[], items, idx) => {
        const validations = [
          ...
          paymentType === PaymentType.DIFFERENT ? DATE_REGEX.test(items[9]) : true,
        ];
          ...
          <div className="h-20 flex justify-between space-x-3 py-3 w-full">
            <div className="flex w-1/2">
              <label className="w-32 stone_btn font-semibold text-center flex justify-center items-center px-3 py-2 cursor-pointer">
                파일 등록
                <input type="file" className="hidden" onChange={(event) => handleFileOnChange(event)} />
              </label>

              <Input type="div" value={selecetdFile} className="rounded-r-lg px-3" />
            </div>
          ...
          <div className="h-2/3 my-3 overflow-y-scroll">
            {listData.length > 0 && selecetdFile ? (
              <table className="w-full table-auto text-center border-stone-100">
                <thead className="*:text-lg font-semibold bg-stone-100 h-16 sticky top-0">
                  <tr>
                    ...
                    <td>담당</td>
                    <td>시작일자</td>
                    {paymentType === PaymentType.DIFFERENT ? <td>납부일자</td> : null}
                  </tr>
                </thead>
                  ...
                  {listData?.map((items: string[], idx) => (
                    <tr
                      ...
                      className={cls(
                        ...
                        errors && errors.includes(idx) ? "bg-orange-200 text-orange-500" : ""
                      )}
                    >
                      {items.map(
                        (item: string, index) => {
                          ...
                          return <td key={`register_member_${idx}-${index}`}>{item}</td>;
                        }
                        // }
                      )}
                    </tr>
              ...
              <div className="w-full h-full flex flex-col justify-center items-center text-sm xl:text-lg xl:space-y-2 space-y-1">
                {listData.length === 0 && selecetdFile ? (
                  <span className="text-base xl:text-xl text-green-600 font-semibold">내용이 작성되지 않았어요!</span>
                ) : null}
                <p>
                  양식을 다운로드하시고,
                  <span className="text-red-600 font-semibold"> 빨간 글씨</span>에 맞게 회원 정보를 입력해주세요
                </p>
                <p>그 다음, 왼쪽 위에 파일을 등록해주시면 등록을 도와드릴게요!</p>
