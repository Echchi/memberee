...
import RegisterWorkers from "../BulkUploadHandlers/worker/registerWorkers";
import Modal from "../../../modal/modal";
import WorkerExcelModal from "../BulkUploadHandlers/worker/workerExcelModal";
import MemberExcelModal from "../BulkUploadHandlers/member/memberExcelModal";
import { getMembers } from "../../../../app/(protected)/member/api";
import InfiniteScroll from "../../../infiniteScroll";
import Loading from "./loading";

const Member = ({ year, month, registerOpen }: { year: number; month: number; registerOpen: boolean }) => {
  ...
  return (
    <>
      {isMemberModalOpen && (
        <Modal
          title={"회원 엑셀 등록"}
          content={<MemberExcelModal onClose={() => setIsMemberModalOpen(false)} />}
      ...
      <div
        ...
        onClick={() =>
          members && members.length > 0 ? router.push("/pay") : registerOpen && setIsMemberModalOpen(true)
        }
                          ...
                          const aHasPayment =
                            a.status === 0
                              ...
                              : a.Payment && a.Payment.length > 0
                                ? a.Payment[0].lessonFee && a.Payment[0]?.lessonFee < 0
                                  ? -1
                                  : 0
                                : 1;
                          const bHasPayment =
                            b.status === 0
                              ...
                              : b.Payment && b.Payment.length > 0
                                ? b.Payment[0]?.lessonFee && b.Payment[0]?.lessonFee < 0
                                  ? -1
                                  : 0
                                : 1;
                        ...
                        .map((member, index) => (
                          <tr
                            ...
                            className={cls(
                              ...
                              member?.Payment && member?.Payment?.length <= 0 ? "bg-orange-50" : ""
                            )}
                          >
                            <td>{member.name}</td>
                            <td>{member.phone}</td>
                            ...
                            <td className="flex justify-center">
                              {member.status === 0 && member.endDate ? (
                                <span className="text-xs">
                                  <Tag color={"stone"} title={`${dateFormattedtoDot(member.endDate)} 탈퇴`} />
                                </span>
                              ) : member.status < 0 ||
                                (member?.Payment &&
                                  member?.Payment[0] &&
                                ...
                                <span className="text-xs">
                                  <Tag color={"yellow"} title={`${dateFormattedtoDot(member.suspendedDate)} 중단`} />
                                </span>
